...
float r = isThickness ? 1.0 : 0.08; //standard deviation amplitude falloff
...
void main()
{
    ...
    for(int i = 0; i < samples; i++){
      for(int j = 0; j < samples; j++){
        offset = vec2(i/(samples - 1.0) - 0.5, j/(samples - 1.0) - 0.5) * (isThickness ? 0.025 : 0.012 * (1.0-depth.x));
